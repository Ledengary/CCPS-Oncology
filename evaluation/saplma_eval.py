#!/usr/bin/env python3
"""
Comprehensive SAPLMA evaluation script.
Loads trained SAPLMA models and evaluates them on test data for response classification.
Supports evaluation on final, middle, or upper-middle layer representations.
"""
...
class SAPLMAEvaluator:
    def __init__(self, model_id: str, trained_models_dir: str, layer_type: str, target_model_version: str = "best"):
        """Initialize the SAPLMA evaluator."""
        ...
        if self.model:
            ...
            self.model.eval()
            logger.info(f"SAPLMA model loaded and moved to device: {self.device}")
    
    def _load_trained_model(self, target_model_version) -> Optional[SAPLMAModel]:
        """Load the best trained SAPLMA model for the specified layer type."""
        model_dir_name = self.model_id.replace('/', '-')
        if target_model_version != "best":
            target_model_version = Path("models") / target_model_version
        model_path = Path(self.trained_models_dir) / model_dir_name / self.layer_type / target_model_version
        ...
        logger.info(f"Loading SAPLMA model with input_dim={input_dim}, hidden_layers={hidden_layers}, dropout={dropout}")
        logger.info(f"Layer type: {self.layer_type}")
        model = SAPLMAModel(input_dim, hidden_layers, dropout)
    ...
    def load_test_representations(self, representations_dir: Path, split_name: str = "test") -> List[Dict[str, Any]]:
        """Load test representations from directory for the specified layer type."""
        model_dir_name = self.model_id.replace('/', '-')
        split_dir = representations_dir / model_dir_name / split_name / self.layer_type
    ...
    def apply_saplma_classification(self, record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Apply SAPLMA classification to a single record."""
        if not self.model:
            logger.error("SAPLMA model not loaded")
            return None

        hidden_state = record['hidden_state']
        
        # Convert to tensor and add batch dimension
        hidden_state_tensor = torch.FloatTensor(hidden_state).unsqueeze(0).to(self.device)

        with torch.no_grad():
            logits = self.model(hidden_state_tensor)
    ...
    def process_test_data(self, representations_dir: Path, split_name: str = "test") -> Tuple[List[Dict[str, Any]], int]:
        ...
        logger.info(f"Processing {split_name} data with SAPLMA classification...")
        ...
        # Process each record
        for record in tqdm(test_records, desc="Applying SAPLMA classification"):
            try:
                result = self.apply_saplma_classification(record)
                if result:
    ...
    def calculate_and_save_results(self, evaluation_records: List[Dict[str, Any]], 
        ...
        # Calculate SAPLMA-specific statistics
        avg_confidence = np.mean(confidence_scores)
        
        # Determine dataset name based on split_name
        dataset_name = "EHRNoteQA" if split_name == "train" else "CORAL-MCQA"
        
        # Add SAPLMA-specific metadata
    ...
    def print_results_summary(self, results: Dict[str, Any], split_name: str) -> None:
        ...
        print(f"\n{split_name.upper()} SAPLMA EVALUATION SUMMARY:")
        print("=" * 60)
        ...
        # SAPLMA statistics
        if 'metadata' in results and 'saplma_statistics' in results['metadata']:
            stats = results['metadata']['saplma_statistics']
            print(f"\nSAPLMA Classification Statistics:")
            print(f"  Layer type: {results['metadata']['layer_type']}")
...
def create_output_path(output_dir: str, model_id: str, layer_type: str) -> Path:
    """Create output directory path."""
    model_dir_name = model_id.replace('/', '-')
    return Path(output_dir) / model_dir_name / layer_type


def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(
        description="Evaluate SAPLMA models for response classification",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    # Input configuration
    parser.add_argument("--representations-dir", type=str, required=True,
                       help="Directory containing test representations (e.g., ../representations/SAPLMA)")
    parser.add_argument("--trained-models-dir", type=str, required=True,
                       help="Directory containing trained SAPLMA models (e.g., ../trained_models/SAPLMA)")
    ...
    parser.add_argument("--layer-type", type=str, required=True,
                       ...
                       help="Which layer to evaluate: final, middle, or upper_middle")
...
def main():
    ...
    # Create output path
    output_path = create_output_path(args.output_dir, args.model_id, args.layer_type)
    ...
    print("  - SAPLMA classification statistics")
...
# Example usage:
# python SAPLMA_evaluation.py \
#     --representations-dir ../representations/SAPLMA \
#     --trained-models-dir ../trained_models/SAPLMA \
#     --model-id "Qwen/Qwen2.5-0.5B-Instruct" \
#     --layer-type final \
#     --output-dir ../results/SAPLMA \
