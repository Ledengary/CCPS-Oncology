#!/usr/bin/env python3
"""
Script to train PIK (Probing for Initial Knowledge) models.
Implements a simple MLP architecture to classify correctness based on
question understanding representations extracted right after seeing the question.
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Train PIK models with static configuration",
...
def pick_tau_by_youden(y_true: np.ndarray, y_prob: np.ndarray) -> float:
    ...
    taus = np.linspace(0.0, 1.0, 1001)  # Fine-grained search
...
def calculate_composite_with_constraints(metrics: Dict[str, float], y_true: np.ndarray, y_prob: np.ndarray,
    ...
    # Use Youden's J for threshold selection
...
class PIKTrainer:
    ...
    def load_hidden_states_from_directory(self, data_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
        """Load all hidden states and labels from a directory of .npz files."""
        hidden_states_list = []
        labels_list = []
        
        npz_files = list(data_dir.glob("*.npz"))
        logger.info(f"Loading data from {len(npz_files)} files in {data_dir}")
        
        for npz_file in npz_files:
            try:
                data = np.load(npz_file, allow_pickle=True)
                hidden_state = data['hidden_state']  # Shape: (hidden_dim,) - single representation per record
                
                # Load correctness (binary 0/1)
                if 'correctness' in data:
                    correctness = data['correctness'].item()  # Extract scalar from numpy array
        ...
        logger.info(f"Loaded {len(all_hidden_states)} samples")
    ...
    def train_model(self, model: PIKModel, train_loader: DataLoader, val_loader: DataLoader,
                    pos_weight_ratio: float, learning_rate: float, weight_decay: float,
                    max_epochs: int = 200, patience: int = 20, grad_clip: float = 1.0) -> Tuple[PIKModel, Dict[str, Any]]:
        """Train a PIK model with early stopping."""
        ...
        # Initialize AMP scaler with new syntax
        ...
        for epoch in range(max_epochs):
            ...
            # Validation phase (now every epoch)
            model.eval()
            val_loss = 0.0
            val_samples = 0
            all_val_preds = []
    ...
    def evaluate_model(self, model: PIKModel, data_loader: DataLoader, return_arrays: bool = False) -> Union[Dict[str, float], Tuple[Dict[str, float], np.ndarray, np.ndarray]]:
        """Evaluate model on a dataset.
        
        Args:
            model: The model to evaluate
            data_loader: DataLoader for evaluation
            return_arrays: If True, also return raw labels and predictions
            
        Returns:
            If return_arrays=False: Dict of metrics
            If return_arrays=True: Tuple of (metrics dict, labels array, predictions array)
        """
        model.eval()
        all_preds = []
        all_labels = []
    ...
    def train_with_config(self, train_hidden_states: np.ndarray, train_labels: np.ndarray,
                         ...
                         output_dir: Path, config: Dict[str, Any]) -> Tuple[Dict[str, Any], PIKModel, Dict[str, Any]]:
        """Train a PIK model with given configuration."""
    ...
    def train_dataset(self, model_dir: Path, output_dir: Path, model_name: str, config: Dict[str, Any]):
        """Train PIK model for a specific model."""
        logger.info(f"\n{'='*60}")
        logger.info(f"Training PIK model for model: {model_name}")
        ...
        hidden_states, labels = self.load_hidden_states_from_directory(train_dir)
...
def main():
    ...
    # Log CUDA setup
    logger.info(f"PIK Training Script with Static Configuration")
    ...
    # Initialize trainer
    trainer = PIKTrainer(device=args.device)
    ...
    for potential_model_dir in representations_base.iterdir():
        if potential_model_dir.is_dir():
            train_dir = potential_model_dir / "train"
            if train_dir.exists() and any(train_dir.glob("*.npz")):
                model_dirs.append(potential_model_dir)
    
    if not model_dirs:
        logger.error(f"No valid model directories found in {representations_base}")
        return
    ...
    for model_dir in model_dirs:
        ...
        if args.model_id is not None:
            # Convert model_id to directory name format
        ...
        try:
            trainer.train_dataset(model_dir, output_dir, model_name, TRAIN_CONFIG)
        except Exception as e:
